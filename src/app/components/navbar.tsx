...
export default function Navbar() {
  ...
  const adminUrl = process.env.NEXT_PUBLIC_ADMIN_URL || "https://admin.mptmamravati.org";
        ...
        <div className="ml-auto hidden md:flex items-center gap-2 lg:gap-3">
          <nav className="flex items-center gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="px-3 lg:px-4 py-3 text-base lg:text-lg font-semibold text-white hover:text-amber-400 transition-colors"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2 ml-2">
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 bg-amber-400/10 hover:bg-amber-400/20 text-amber-300 hover:text-white font-bold text-xs sm:text-sm px-4 py-2 rounded-full border border-amber-400/50 shadow-sm hover:border-amber-400 hover:scale-105 active:scale-95 transition-all duration-200 shrink-0"
            >
        ...
        <nav className="flex flex-col border-t border-amber-500/30 bg-[#3a0303] px-4 sm:px-6 py-2">
          {navLinks.map((link) => (
            <Link
              ...
              className="px-2 py-3 text-base font-semibold text-white hover:text-amber-400 border-b border-amber-500/10 last:border-b-0 transition-colors"
            >
              {link.label}
            </Link>
          ))}
          ...
          <div className="flex items-center gap-2 my-3">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 text-center inline-flex items-center justify-center gap-1.5 bg-amber-400/10 hover:bg-amber-400/20 text-amber-300 hover:text-white font-bold text-xs sm:text-sm px-3 py-2.5 rounded-full border border-amber-400/50 transition-all"
            >
              <LogIn className="w-3.5 h-3.5 text-amber-400" />
              <span>लॉगिन</span>
            </a>
